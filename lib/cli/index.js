...
module.exports = class CLI {
  ...
  start ([repoPath]) {
    ...
    // Run commands
    const commands = [
      new Command (`git clone --depth 1 https://github.com/beezwax/fmbond-web-template ${repoPath}`, undefined, {message: `\x1b[33mCreating repo ${repoName}\x1b[0m`}),
      new Command (`cd ${repoPath} && npm install`),
      new Command (`cd ${repoPath} && npm run install_fm_suite ./src/filemaker/${repoName}.fmp12`),
      new Command (
        process.platform === "darwin" ?
          `cd ${repoPath} && rm -rf .git .gitignore` :
          `cd ${repoPath} && rmdir /s /q .git && del .gitignore`
      ),
