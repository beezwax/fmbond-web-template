  ...
  const run = async () => {
    ...
    if(port) {
      ...
      openFmFile(repoPath).then(() => {
        setTimeout(() => upsertFmWebProject(repoName), 500);
        setTimeout(() => runFmWebProject(repoName, port), 600);
        setTimeout(() => console.log("\x1b[32mWeb project opened in FileMaker!\x1b[0m"), 700);
      });
    } else {
